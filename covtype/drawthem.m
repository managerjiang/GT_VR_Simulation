...
%% meth3--------------------------------------------------------------%%%%%%%%%%%%
% ==============需要读取.mat文件================
  load('data/G_meth4_800.mat');%加载保存的迭代梯度信息x_k_store{}
  load('data/X_meth4_800.mat');%加载保存的迭代解信息gradient()
...
%% meth4
% ==============需要读取.mat文件================
  load('data/G_meth5_800.mat');%加载保存的迭代梯度信息x_k_store{}
  load('data/X_meth5_800.mat');%加载保存的迭代解信息gradient()
%  load('data/C_meth3_smote_800.mat');%加载保存的邻接矩阵信息C_store
 load('cov.mat');
  load('L_cov.mat');
  A(435756:581009,:)=[];
...
tt=1:20:Maxgen00;
subplot(1,2,1)
plot(tt(1:4:length(tt)),w_m1(1:4:length(tt)),'r-x','linewidth',1),hold on;
plot(tt(1:4:length(tt)),w_m2(1:4:length(tt)),'b-.','linewidth',1),hold on;
plot(tt(1:4:length(tt)),w_m4(1:4:length(tt)),'k-^','linewidth',1),hold on;
plot(tt(1:4:length(tt)),w_m5(1:4:length(tt)),'m-o','linewidth',1),hold on;
 legend('GT-VR','GT-SAGA','GT-SARAH','D-GET');
ylabel('$$\|D(\bar{x})\|$$','Interpreter','latex')
xlabel('iterations');
 subplot(1,2,2)
 plot(tt(1:4:length(tt)),obj_m1(1:4:length(tt)),'r-x','linewidth',1),hold on;
 plot(tt(1:4:length(tt)),obj_m2(1:4:length(tt)),'b-.','linewidth',1),hold on;
   plot(tt(1:4:length(tt)),obj_m4(1:4:length(tt)),'k-^','linewidth',1),hold on;
 plot(tt(1:4:length(tt)),obj_m5(1:4:length(tt)),'m-o','linewidth',1),hold on;
 legend('GT-VR','GT-SAGA','GT-SARAH','D-GET');
